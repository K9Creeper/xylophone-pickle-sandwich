...
_context_switch_kernel:
    ...
    ; Set up segment registers for kerenel mode (0x10 = Kernel Data Segment)
    mov ax, 0x10
    mov ds, ax
    mov es, ax
    mov fs, ax
    ...
    ; Load ESP (Kernel Stack Pointer)
    mov eax, [ebp + 16]  ; eax = kerenel stack pointer
    ...
    ; Load CS and EIP
    push 0x08            ; Kernel Code Segment (0x08)
    mov eax, [ebp + 44]  ; eax = eip (entry point)
    push eax             ; Push EIP
    ...
    ; Perform iret (Interrupt Return) to switch to kernel mode
    iret
