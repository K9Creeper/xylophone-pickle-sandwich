...
static void *allocate(heap_manager_t *heap, uint32_t size, uint8_t should_align, uint8_t iterration);
static void unallocate(heap_manager_t *heap, uint32_t address);

static uint8_t is_less_than(const type_t a, const type_t b)
{
    return (((heap_header_t *)(a))->size < ((heap_header_t *)(b))->size);
}

void heap_manager_bootstrap(heap_manager_t *heap, uint32_t end)
{
    if (!heap)
        return;
    ...
    heap->status = HEAP_BOOTSTRAPED;
}

extern void heap_manager_init(heap_manager_t *heap, uint32_t start_address, uint32_t end_address, uint32_t max_end_address,
                              uint8_t is_supervisor, uint8_t is_readonly, uint8_t use_identity_mapping,
                              struct paging_manager_s *paging_manager)
{
    if (heap == NULL)
        return;

    if (heap->status != HEAP_BOOTSTRAPED)
        return;
...
void *heap_manager_malloc(heap_manager_t *heap, uint32_t size, uint8_t should_align, uint32_t *physical_address)
{
    ...
    else if (heap->status == HEAP_BOOTSTRAPED)
    {
...
static uint32_t contract(heap_manager_t *heap, uint32_t new_size)
{
    ...
    {
        paging_manager_free_single(heap->paging_manager, i, 1);
    }
